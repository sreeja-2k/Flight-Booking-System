...
if ($completed = $mysqli->query("SELECT *, T.Price AS finalPrice, (SELECT Name FROM `airports` WHERE Id = Source) AS SourceAirport, (SELECT Name FROM `airports` WHERE Id = Destination) AS DestinationAirport FROM `tickets` T JOIN `flights` F ON F.Id = T.FlightId WHERE DateTime <= CURDATE();")) {
}
?>
